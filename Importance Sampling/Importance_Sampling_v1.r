...
result<-data.frame(matrix(nrow=10,ncol=3))
colnames(result)<-c("True.p","IS","IS.t.test")
# l'exemple de EXPERT
set.seed(1)
...
t.obs <- F(z, L0);
# True p-value
result[1,1]<- 2.0*(1-pt(t.obs, df=length(z)- 2))


result[1,2]<-Run_IS(F, z, L0, DIST, 1e5, 0.8, 2)$p
result[1,3]<-Run_IS_t_test(z, L0, DIST, 1e5, 0.8, 2)$p
